...
def wait_pin_change(pin_check):
    # wait for pin to change value
    # it needs to be stable for a continuous 20ms
    global flag_queue
    cur_value = pin_check.value()
    active = 0

    while active < 20:
        if pin_check.value() == cur_value: # was !=
            active += 1
        else:
            active = 0
        print(active) 
        time.sleep_ms(1)
    flag_queue.insert(0, pin_check)
...
def main_loop(**kwargs):
    ...
    while True:
        ...
        if pin_flag:
            light.low()
            print("if pin_flag: ", pin_flag)
            wait_pin_change(pin_flag)
            #flag_queue.insert(0, pin_check)
            pin_flag = None
            curr_value = None

        if flag_queue:
            print("if flag queue: ", flag_queue)
